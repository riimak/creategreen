...
class TransactionBuilder {
  ...
  constructor(network = networks.bitcoin, maximumFeeRate = 2500) {
    ...
    // FEELESS customization
    this.__TX.version = 4;
    this.__USE_LOW_R = false;
    if (process.env.BLOCKCHAIN_SHOW_VENDOR_WARNINGS === 'true') {
      console.warn(
  ...
  addInput(txHash, vout, sequence, prevOutScript) {
    ...
    if (txIsString(txHash)) {
      ...
      const txOut = txHash.outs[vout]; // nosemgrep detect-bracket-object-injections
...
function getSigningData(
  ...
  const input = inputs[vin]; // nosemgrep detect-bracket-object-injection
